#!/usr/bin/env python3
"""
forgery_models_5version_compare_fixed_complete.py

Complete script: trains/evaluates multiple models and includes robust dataset handling.
- TinyCNN
- ResNet-50 (finetune)
...
Fixes included:
- Robust build_classification_datasets (accepts ImageFolder layout OR flat folder with heuristics)
- Fixed regex in fuzzy_pair_images_labels
- Simpler segmentation forward handling (supports dict outputs)
- Minor print fixes
...
INITIAL_LR = 1e-4
L2_REG = 1e-4
INPUT_SIZE = (360, 480)        # classifier input (H,W)
...
def train_classifier(model, train_loader, val_loader, num_epochs, lr, save_path=None):
    ...
    for epoch in range(num_epochs):
        model.train()
        running_loss = 0.0
        for imgs, labels, _ in tqdm(train_loader, desc=f"Train epoch {epoch+1}/{num_epochs}"):
            imgs = imgs.to(DEVICE)
        ...
        with torch.no_grad():
            for imgs, labels, _ in val_loader:
                ...
                total += labels.size(0)
        val_loss = running_loss / len(val_loader.dataset) if len(val_loader.dataset)>0 else 0.0
...
def evaluate_classifier(model, data_loader, class_names):
    ...
    paths = []
    with torch.no_grad():
        for imgs, labels, pths in data_loader:
            imgs = imgs.to(DEVICE)
            outputs = model(imgs)
...
def plot_rocs(scores, y_true, class_names, out_path_prefix):
    ...
    for c in range(len(class_names)):
        try:
            ...
            continue
    plt.plot([0,1],[0,1],'k--')
    plt.xlabel('FPR'); plt.ylabel('TPR'); plt.title('ROC curves (one-vs-rest)'); plt.legend(loc='best')
    plt.grid(True)
