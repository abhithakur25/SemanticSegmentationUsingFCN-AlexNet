...
RESULTS_FOLDER = os.path.join(DATA_ROOT, 'mobilenetv2_results')
...
DEVICE=torch.device('cuda'); print('GPU:', torch.cuda.get_device_name(0))
...
def build_ds(root):
    ...
    if imf is None:
        ...
        paths=[p for p,_ in inf]; labels=[idx[l] for _,l in inf]
    # filter
    p2,y2=[],[]
...
def make_model(nc):
    ...
    net.classifier[1] = nn.Linear(net.classifier[1].in_features, nc)
    return net
...
def train(model, tr, va):
    ...
    best=0.0; savepath=os.path.join(RESULTS_FOLDER,'mobilenetv2_best.pth')
    for ep in range(EPOCHS):
        model.train(); run=0.0;n=0
        for x,y in tr:
            x=x.to(DEVICE); y=y.to(DEVICE)
            opt.zero_grad(); o=model(x); l=crit(o,y); l.backward(); opt.step()
            run+=float(l.item())*x.size(0); n+=x.size(0)
        sch.step()
...
def plot_cm(cm, classes, path):
    plt.figure(figsize=(5,4)); plt.imshow(cm); plt.title('MobileNetV2 Confusion'); plt.colorbar()
...
def main():
    ...
    plot_cm(cm,classes,os.path.join(FIG_FOLDER,'confusion_mobilenetv2.png'))
    with open(os.path.join(RESULTS_FOLDER,'metrics_mobilenetv2.json'),'w') as f:
